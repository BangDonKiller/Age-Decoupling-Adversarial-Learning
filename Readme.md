...
5. pca_variance_vs_gender_correlation: PCA 主成分所涵蓋的變異量與目標特徵(性別)的相關程度
6. analyze_gender_separability_sb_sw: 計算目標特徵的組內距離以及組間距離，觀察模型是否可以能夠把目標特徵分得夠開(組間)以及計算除了目標特徵以外的樣本點變異量(組內)，越大代表性別以外的資訊影響越大
7. test: 做了 KNN, LR, silhouette 分數分析，觀察目標特徵於嵌入空間中的分布以及線性可分程度
8. analyze_embedding_pca_snr_gender: 觀察目標特徵(目前為性別)、環境因子與主成分的相關程度