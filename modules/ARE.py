...
#         # 計算空間維度上的均值，對倒數第二個和倒數第一個維度（即頻率和時間維度）求均值
#         mean = x.mean(dim=[-2, -1], keepdim=False)

#         # 計算空間維度上的標準差，添加一個很小的常數 (eps) 是為了數值穩定性，防止標準差為零導致除以零
...
class AttentiveStatisticalPooling(nn.Module):
    def __init__(self, input_channels, attn_hidden=128):
        super().__init__()
        self.attn = nn.Sequential(
            nn.Conv2d(input_channels, attn_hidden, kernel_size=1),
            nn.ReLU(),
    ...
    def forward(self, x):
        # x: (B, C, F, T)
        B, C, F, T = x.shape
        attn = self.attn(x)  # (B,1,F,T)
        attn = attn.view(B, -1)
        weight = torch.softmax(attn, dim=1).view(B,1,F,T)
        weighted = x * weight
        mean = weighted.sum(dim=[2,3])  # (B, C)
        sq = (weighted * x).sum(dim=[2,3])  # (B, C)
        std = torch.sqrt(sq - mean * mean + 1e-9)
        return torch.cat([mean, std], dim=1)
